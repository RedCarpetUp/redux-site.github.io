...
const GovtDocsXiaomi = (props) => {
  ...
  const getUserStatus = async () => {
    ...
    if (response.result == "success" && response.data.length > 0) {
      if (["rejects"].includes(response.data[0].current_task)) {
        moveRebel(selectedProduct);
      } else if (["pan_verification"].includes(response.data[0].current_task)) {
        setUserStatus(true);
      }
    } else {
  ...
  useEffect(() => {
    let analyticData = {
      ...
      "Screen Name": analyticsData.XIAOMI_ONBOARDING_SCREEN,
    };
  ...
  const verifyOtp = async () => {
    try {
      setIsLoading(true);
      let response = await callApi(
        "/verify_aadhar_id_redcarpet_xiaomi_customer",
        "POST",
        {
          otp: OTP,
          type: "verify",
      ...
      if (response.result === "success") {
        let analyticData = {
          Event: analyticsData.AADHAR_VERIFICATION_SUCCESS_XIAOMI,
        };
        useAnalytics(analyticData);
        ...
        let analyticData = {
          Event: analyticsData.AADHAR_VERIFICATION_ERROR_XIAOMI,
        };
        useAnalytics(analyticData);
        notify({ message: response.message, type: "error" });
      }
  ...
  const setUserPan = async () => {
    ...
    try {
      setIsLoading(true);
      let response = await callApi(
        "/set_pan_redcarpet_xiaomi_customer",
        "POST",
        {
          pan: pan,
        },
      ...
      if (response.result === "success") {
        ...
        setIsLoading(true);
        let response1 = await callApi(
          "/accept_user_agreement_esign_redcarpet_xiaomi_customer",
          "GET",
          {},
          phone,
          accessToken
  ...
  const SignUser = async () => {
    ...
    try {
      setIsLoading(true);
      let response = await callApi(
        "/generate_user_esign_redcarpet_xiaomi_customer",
        "GET",
        {},
        phone,
        accessToken
      ...
      if (response.result == "success") {
        let analyticData = {
          Event: analyticsData.PAN_VERIFICATION_SUCCESS_XIAOMI,
        };
        useAnalytics(analyticData);
        try {
          if (dataBranch["~id"]) {
      ...
      <div className="row">
        {isLoading ? (
            ...
            <div className="section-dialog">
              <div className="form Input-line">
                <div className="row-left">
                  ...
                  <div className="col70">
                    <h1 style={{ marginLeft: 5 }}>MI Credit Documents</h1>
                  </div>
                </div>
                {otpScreen == "1" ? (
                  <Fragment>
                ...
                {aadharVerified ? (
                  <span className={"inputPayCombo-note"}>
                    <div className="note-red">
                      If you have verified your aadhar details before then they
                  ...
                  <span className={"inputPayCombo-note"}>
                    ...
                    <span className="note-txt">
                      Submitting Pan Card Number for MI Credit is mandatory for
          ...
          <div className={"col60"}>
            ...
            <Text content="Please download and read the Lender agreement for MI Credit below carefully before proceeding." />
            {lender
              ? lender.map((lend) => {
                  return (
                    <div className={"col80"}>
                ...
                <label for="tnc">
                  <div className="row">
                    ...
                    <div
                      ...
                      className="col90 lender-agreement"
                    >
